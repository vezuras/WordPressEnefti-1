...
function download_and_insert_image_to_wp_media_library($image_url, $annonce_id) {
    ...
    if ($image_data === false) {
        echo "Impossible de télécharger l'image depuis l'URL : $image_url\n";
    ...
    if ($result === false) {
        echo "Erreur lors de l'enregistrement de l'image : $file_path\n";
    ...
    if (!is_wp_error($attach_id)) {
        ...
        echo "Erreur lors de l'insertion de l'image dans la Media Library : " . $attach_id->get_error_message() . "\n";
...
/**
 * Fonction pour extraire la ville à partir de l'adresse pour LogisQuébec
 *
 * @param array $annonce Données de l'annonce
 * @return string Nom de la ville ou 'Ville non disponible'
 */
function extract_city_from_logisqc($annonce) {
    // Extraire la ville à partir de la clé 'locality'
    if (!empty($annonce['address']['locality'])) {
        return trim($annonce['address']['locality']);
    }

    // Si aucune ville n'est trouvée, fallback sur 'unknown city'
...
try {
    ...
    // Vérification de la collection 'logisqc_full'
    $collection = $database->selectCollection('logisqc_full');
    ...
    if (!empty($annonces)) {
        ...
        foreach ($annonces as $annonce) {
            ...
            if (!empty($annonce['image_url'])) {
                ...
                if (is_null($image_id)) {
                    echo "Échec du téléchargement ou de l'insertion de l'image pour l'annonce_id {$annonce['_id']}.\n";
                }
            }

            // Extraire la ville de l'annonce
            $city = extract_city_from_logisqc($annonce);
            ...
            $annonce_data = [
                ...
                'type' => 'external',
                'regular_price' => isset($annonce['price']) ? number_format((float)$annonce['price'], 2, '.', '') : '0.00', // Utilisation du prix de l'annonce
                'description' => !empty($annonce['description']) ? "<p><strong>Description complète :</strong> " . htmlspecialchars($annonce['description']) . "</p>" : '', // Description longue
                ...
                'button_text' => 'Voir sur LogisQuébec', // Modification du texte du bouton
                'meta_data' => [
        ...
        echo "Aucune annonce trouvée dans la collection 'logisqc_full'.\n";
    }
